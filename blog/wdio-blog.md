# Meta:
- Title: React Native UI testing with WDIO

- Keywords: React Native, UI testing, WDIO, Appium, SauceLabs, Real Device testing

- Summary: We recently adopted WDIO based UI testing for a React Native application. Benefits of using WDIO includes you can write UI tests similiar to how you write for web. 
  Synchronous nature of WDIO helps write UI tests that are clean. WDIO config allows to plugin SauceLabs's Emulators or Real Devices for cloud based testing.

- Author: Raja Panidepu.

# Article:

## Motivation: 
I work on a Mobile App building team which employ React Native Framework. Similar to how we write JS code to develope components for Android or iOS platform, we wanted to write our UI tests once and run for both the platforms. The answer we found is WebDriverIO.

WebDriverIO is Node.js implementation of WebDriver Protocol. WebDriverIO is most popular WebUI testing framework, which also supports Native Mobile UI testing.

The next part of the article discusses about setting up WebDriverIO of the following scenarios:
...
Lets install WebDriverIO globally and create a basic WebDriverIO config.
...
WebDriverIO supports multiple services, of which Appium is test automation framework for use with native, hybrid and mobile web apps. [WebDriverIO's Appium service](http://webdriver.io/guide/services/appium.html) lets you automagically run Appium server in the background, which passes on UI test commands to the mobile simulator/emulator.
...
In the above config, we have added `appium` to the services list, updated port number to point to Appium's default port number. Notice that we have removed `baseUrl` field as we dont need it.

Capabilites have a `app` field whose value should be set with path to the `.apk` for Android or `.app` iOS application. Usual location would be `<PROJECT_ROOT>/android/app/build/outputs/apk/<FILE_NAME.apk>` or `<PROJECT_ROOT>/ios/build/Build/Products/Debug-iphonesimulator/<FILE_NAME.app>`

We also set `maxInstances` to 1 when running locally to avoid running out of memory, which is the cae when we spin up more than 1 emulator.
...
Above is a basic dummy test block that installed the application on the emulator, opens the app, pauses for 2 seconds, and closes the application.

For newbies, `browser` is set as global variable by WebDriverIO [here](https://github.com/webdriverio/webdriverio/blob/master/lib/runner.js#L46)

Finally, run the test:
For iOS and Android, make sure you have needed xcode and Android Studio build tools setup to run React Native application.

For Android, start the emulator before running the tests.
...
Lets configure our current WDIO test to run using SauceLabs Simulators.
...
Hope the above discussion helps you setup some infrastructure for running WDIO UI tests locally and remotely for Emulators/Real devices. 
...
A test is declared as failure if it isn't completed within a certain time. Test framework and their respective timeout options are listed below.
```js
// Mocha
mochaOpts: {
  timeout
...
**Appium timeouts:** 
These timeouts deal with how long appium should wait for the emulator/simulator to be ready and time taken for the app to be installed.
- `appWaitDuration`
- `deviceReadyTimeout`
